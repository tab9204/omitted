...
var reminders = {
  //all the users reminders
  allReminders: [],
  //gets all reminders for the user and sorts them
  getAll: async ()=>{
    //empty out the array
    reminders.allReminders = [];
    //query the db for all the reminders
    var all = await database.getAllReminders();
    //loop through the reminders returned
    for(var i = 0; i < all.length; i++){
      reminders.allReminders.push(all[i].details);
    }
    //sort the arrays based on timestamp in acending order
    reminders.allReminders.sort((a,b) => a.timeStamp - b.timeStamp);
  },
